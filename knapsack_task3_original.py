...
def solve_by_dominance_branch_and_bound(items, capacity):
    ordered = sort_items_for_search(items)
    best_value = 0
    best_weight = 0
    best_ids = []
    node_count = 0
    frontier_by_depth = [[] for _ in range(len(ordered) + 1)]
    ...
    def is_dominated(depth, current_weight, current_value):
        for saved_weight, saved_value in frontier_by_depth[depth]:
            if saved_weight <= current_weight and saved_value >= current_value:
                return True
        return False

    def register_state(depth, current_weight, current_value):
        next_frontier = []
        for saved_weight, saved_value in frontier_by_depth[depth]:
            if current_weight <= saved_weight and current_value >= saved_value:
                continue
            next_frontier.append((saved_weight, saved_value))
        next_frontier.append((current_weight, current_value))
        frontier_by_depth[depth] = next_frontier
    ...
    def dfs(index, current_weight, current_value, chosen):
        ...
        if is_dominated(index, current_weight, current_value):
            return

        # 同じ深さで既に負けている状態を消し、非支配状態だけを残す。
        register_state(index, current_weight, current_value)
...
def measure_dominance_branch_time(items, capacity, trials=20):
    elapsed_list = []
    result = None

    for _ in range(trials):
        start_time = time.perf_counter()
        result = solve_by_dominance_branch_and_bound(items, capacity)
...
def analyze_case(items, capacity):
    ...
    branch_choice, branch_ms = measure_dominance_branch_time(items, capacity)
    return {
...
def print_comparison(gap):
    ...
    print("利得差 =", gap)
    if gap == 0:
        print("判定 = 提案手法でも最適解に到達")
    else:
        print("判定 = 提案手法は最適解に到達していない")
...
def print_time_summary(summary_rows):
    print(SEPARATOR)
    print("製品数と実行時間のまとめ")
    print("製品数 容量制約 完全列挙法ms 提案手法ms 探索ノード数 利得差")
    for row in summary_rows:
        print(
            f"{row['item_count']:>4} {row['capacity']:>8} "
            f"{row['exhaustive_ms']:>12.3f} {row['branch_ms']:>10.3f} "
...
def print_large_time_summary(summary_rows):
    ...
    print("製品数 平均容量 提案手法ms 平均探索ノード数")
    for row in summary_rows:
        print(
            f"{row['item_count']:>4} {row['capacity']:>8.1f} "
            f"{row['branch_ms']:>10.3f} {row['node_count']:>16.1f}"
        )
...
def run_large_size_comparison():
    ...
    for item_count in LARGE_SIZE_LIST:
        ...
        for seed in LARGE_SEED_LIST:
            ...
            branch_choice, branch_ms = measure_dominance_branch_time(case["items"], case["capacity"], trials=10)
